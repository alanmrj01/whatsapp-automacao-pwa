...
test('niche UX keeps the approved priority and technical context', () => {
  ...
  assert.match(conversations, /Conversas prioritárias e não lidas aparecem primeiro/i)
  assert.match(conversations, /Buscar conversa/i)
  assert.match(dashboard, /Visão do dia/)
  assert.match(agenda, /Dados de demonstração/)
...
test('preview uses the approved company name, waiting label and service icons', () => {
  const preview = read('src/features/preview/PlatformPreviewPage.tsx')
  const demo = read('src/demo/operationalDemo.ts')
  assert.doesNotMatch(preview, /PEMA TESTE|>Leads\s/)
  assert.match(demo, /demoBusinessName = 'PEMA Ar Condicionado'/)
  assert.equal(preview.match(/demoBusinessName/g)?.length, 3)
  assert.match(preview, /Aguardando atendimento <span>\{demoOverview\.waiting\}<\/span>/)
